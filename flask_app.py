...
def drohne_loeschen(id):
    if current_user.role != 'mitarbeiter': return "Verboten!"
    # FIX: Zuerst abhängige Bestellungen löschen
    db_write("DELETE FROM Bestellung WHERE Drohnen_id = %s", (id,))
    # Dann die Drohne löschen
...
def drohne():
    ...
    # FIX: Wir holen den Preis jetzt direkt aus der DB, da das Formular anscheinend immer 10 schickt.
    drohnen_id = request.form["drohnen_id"]
    db_daten = db_read("SELECT Drohnen_Preis_CHF FROM Drohnen WHERE Drohnen_id = %s", (drohnen_id,))
    
    # Wir nehmen an, db_read gibt eine Liste von Dictionaries zurück (Standard in dieser Vorlage)
    echter_preis = db_daten[0]["Drohnen_Preis_CHF"]

    session["drohne_id"] = drohnen_id
    session["drohne_preis"] = float(echter_preis)
...
def drohne_automatisch_freigeben(drohnen_id, app):
    # FIX: App-Context nutzen, damit DB-Verbindung im Thread klappt
